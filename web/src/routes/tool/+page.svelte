...
<svelte:head>
	...
	<meta name="description" content="A full jed database tool in your browser: SQL editor, schema, and local databases via OPFS." />
</svelte:head>
...
<div class="mb-3">
	...
	<p class="text-sm text-jed-muted">
		...
		or export single-file <code>.jed</code> databases stored in your browser's private file system
		(OPFS).
	</p>
</div>
...
<div class="grid grid-cols-1 gap-4 lg:grid-cols-[16rem_minmax(0,1fr)]">
	...
	<aside class="space-y-4">
		<section class="rounded-lg border border-slate-200 bg-white" data-testid="schema-sidebar">
			<h2 class="border-b border-slate-200 px-3 py-2 text-xs font-semibold tracking-wide text-jed-muted uppercase">
				Schema
			</h2>
			...
			<div class="p-3 text-sm">
				...
				{:else}
					...
					<ul class="mt-2 space-y-1" data-testid="file-list">
						{#each files as f (f.name)}
							<li class="flex items-center gap-1 text-xs">
								...
								<button onclick={() => exportFile(f.name)} disabled={busy} class="text-slate-500 hover:text-jed-ink" title="export">↓</button>
								<button onclick={() => dropFile(f.name)} disabled={busy} class="text-slate-400 hover:text-red-600" title="delete">✕</button>
							</li>
						{:else}
					...
					<label class="mt-2 block cursor-pointer text-xs text-jed-accent hover:underline">
						Import a .jed file…
						<input type="file" accept=".jed" class="hidden" onchange={importFile} data-testid="import-input" />
					</label>
				{/if}
				{#if fileMsg}
	...
	<div class="space-y-3">
		<div class="flex flex-wrap items-center gap-3 text-sm">
			...
			{#if isOpfs}
				<button onclick={useMemory} class="text-xs text-jed-accent hover:underline">switch to in-memory</button>
			{/if}
		...
		<div class="flex flex-wrap items-center gap-2">
			...
			<button onclick={commit} disabled={!db} data-testid="commit-button" class="rounded-md border border-slate-300 px-3 py-1.5 text-sm hover:bg-slate-50">Commit</button>
			<button onclick={rollback} disabled={!db} data-testid="rollback-button" class="rounded-md border border-slate-300 px-3 py-1.5 text-sm hover:bg-slate-50">Rollback</button>
		...
		{:else if formatted}
			<div class="not-prose">
				<div class="mb-1 flex justify-end">
					<button onclick={copyFormatted} class="text-xs text-jed-accent hover:underline">copy</button>
				</div>
				<pre
