...
<svelte:head>
	...
	<meta name="description" content="CREATE TABLE with typed columns, PRIMARY KEY, NOT NULL, CHECK and UNIQUE constraints — enforced live." />
</svelte:head>

# Tables & constraints

`CREATE TABLE` declares typed columns and constraints: `PRIMARY KEY`, `NOT NULL`, `DEFAULT`,
`CHECK`, and `UNIQUE`. Constraints are enforced on every write, with a structured error code when
violated.

The table below was created with a `CHECK (balance >= 0)` and a `NOT NULL` owner. Run the query, then
try editing it to break a constraint:
