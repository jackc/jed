...
MAGIC = "JEDB".b
VERSION = 15 # format_version 15: IDENTITY columns — the column-entry flags byte gains bit4
# is_identity + bit5 identity_always (GENERATED ALWAYS set / GENERATED BY DEFAULT clear). An identity
# column desugars exactly like serial (an owned sequence + a nextval expression default + NOT NULL),
# so the only on-disk change is those two bits; the bits restore the INSERT/UPDATE 428C9 gating after a
# reopen (spec/design/sequences.md §13). format_version 14 was the serial OWNED-sequence link — the
# sequence-entry flags byte gains bit2 has_owner; when set, the flags byte is followed by the owner
# reference (owner table name u16-len + bytes, then owner column ordinal u16). A non-owned sequence
# writes nothing after the flags byte (the v12 shape). The link records the OWNED BY relationship a
# serial column establishes, so a reopened database still auto-drops the owned sequence on DROP TABLE
