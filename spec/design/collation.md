...
>
> **Status: design ratified; slices 1a + 1b landed (host-free compiler + executor + artifact codec
> in all three cores); no SQL surface or persistence yet.** No core ships a non-`C` collation in
> the SQL surface today; `text` is `C` (byte / code-point order) and needs **zero data**
> ([types.md §11](types.md)). **Slice 1a** landed the byte-format foundation in
> [../collation/README.md](../collation/README.md) (the definition format, the compiled-table
> layout, the `.coll` artifact, the sort key) plus the dev fixtures. **Slice 1b** landed the
> implementation — `CompileCollation`, the `sortKey` executor, and `SaveCollation`/`OpenCollation`
> — in `impl/{rust,go,ts}`, byte-identical and pinned by the populated
> [../collation/vectors/](../collation/) (compiler + sort-key vectors generated by the Rust core
> and cross-confirmed by Go and TS). The remaining slices (§14) land against them. Two decisions
> are **confirmed**: the definition format is the **UCA/CLDR standards** (DUCET `allkeys.txt` +
> LDML), and the cores build **`CompileCollation` in-core first** (no external authoring tool).
> This doc pins no `format_version` or `type_code` number: the version bump and the new catalog
...
- **Bounded cost.** Sort-key generation is metered by a `collate` cost unit per code point
  ...
  large input is cost-ceilinged ([cost.md](cost.md)). The unit joins the shared cost schedule in
  **1c**, when the executor is wired into query execution and there is a metering site (1b's
  `sortKey` is a pure function).
