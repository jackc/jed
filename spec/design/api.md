...
- **A spill-target override** — jed already has the `work_mem` *budget* (`open` opts, §2.1) and native
  file hosts default spills to host scratch storage (the OS temp directory, independent of the
  database path; [spill.md](spill.md) §4), but callers cannot yet select another location. DuckDB's
