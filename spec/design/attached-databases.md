...
honest model. (Multi-process file access is now spec'd in [locking.md](locking.md): every file
attachment joins its own per-file shared coordination bundle at attach time, independently of
`main`; the existing one-durable-writer rule remains until this document's super-journal slice.)
