...
5. `unify(elem, X)`: if `elem` is unbound, set it to `X`; if already bound, require structural
   type equality (`i32 == i32`, `text == text`, composite by catalog ref) — a conflict
   (`array_cat(i32[], text[])`) makes the overload **not match**.
...
**Literal adaptation (two-pass resolution).** jed types an undecorated integer literal as `i64`
(CLAUDE.md §8 / types.md §6), where PostgreSQL types a small one as `int4`. So `array_append(xs,
40)` over an `i32[]` column would, naively, see element `40` as `i64` and reject the overload.
...
adapts — with a range check — to the array's element type. `array_append(i32[], 40)` lands on
`i32` (`array_append(i32[], 5000000000)` is `22003`), and `array_cat(i32[], ARRAY[7,8])`
adapts the constructor to `i32[]`. The hint is the element type of the **first** `anyarray`
...
|---|---|---|---|
| `array_ndims` | `(anyarray)` | `i32` | `propagates` |
| `array_length` | `(anyarray, integer)` | `i32` | `propagates` |
| `array_lower` | `(anyarray, integer)` | `i32` | `propagates` |
| `array_upper` | `(anyarray, integer)` | `i32` | `propagates` |
| `cardinality` | `(anyarray)` | `i32` | `propagates` |
...
The dimension argument's family is **`integer`** (jed's integer family — i16/i32/i64). PG's
is strictly `int4`, so jed accepts `array_length(xs, 1::i64)` where PG raises `42883`; a **minor
documented divergence** (§5), invisible to the corpus (integer literals resolve fine). The result
is always **`i32`** (PG's `integer`), regardless of the array's element type.
...
3. **The dimension argument is the `integer` family, not strict `int4`** — `array_length(xs,
   1::i64)` is accepted (PG: `42883`). A minor divergence following jed's existing integer-family
...
8. **Element unification adapts literals to the *first* `anyarray` argument** — the literal-adaptation
   ...
   (narrower) element type second (`array_cat(ARRAY[1,2], i32[]_col)` → `42883`), where PG would
...
| Failure | Code |
|---|---|
| No array-function/`\|\|`/`@>`/`<@`/`&&`-overload matches the argument types (incl. element-type conflict `array_cat(i32[], text[])`, a non-array where `anyarray` is required, or text/`int\|\|int` `\|\|`) | `42883` undefined_function |
...
**typed** null element (`arr || NULL::i32`) is a concrete `anyelement`, never defers, so cat
...
| `array_position` | `(anyarray, anyelement [, integer])` | `i32` | first match's **subscript** (lower-bound space), else `NULL`; **1-D/empty only** (`0A000`); optional `start` subscript, NULL `start` is `22004` |
| `array_positions` | `(anyarray, anyelement)` | `i32[]` | the `i32[]` of **every** match's subscript (empty `{}` if none); **1-D/empty only** (`0A000`) |
...
hint AF1 uses, so `array_remove(i32[]_col, 2)` adapts `2`.

`array_position`/`array_positions` return jed's `i32` / `i32[]` (PostgreSQL's `integer` /
`integer[]`) regardless of the array's element type, matching the §3.1 introspection convention.
`i32[]` is a new concrete-array `result` code (`<scalar>[]`), read by the resolver as
...
2. resolve the single argument with **no hint** (the argument *is* the array; there is no element to
   ...
   **array** `E[]` → bind `ELEM := E`, the output column type. A **non-array** (`unnest(5)`) is
   ...
   NULL array (`NULL::i32[]`) resolves and yields zero rows at exec.
3. `unnest`'s `ELEM` may be a **scalar or a composite** element (AF7, §13); the synthetic column
   carries it (`i32[]` → an `i32` column, `text[]` → `text`, `addr[]` → an `addr` composite
...
3. **Strict on a NULL whole-array operand.** `null = "propagates"`: a NULL array operand (the whole
   value, e.g. `NULL::i32[] @> ARRAY[1]`) yields **NULL**, the ordinary strict short-circuit. This
...
2. compute the element hint from the **first operand that is an array** and re-resolve the **non-NULL**
   ...
   (`i32[]_col @> ARRAY[20]` lands on `i32`, exactly the §8.1 literal adaptation) — a bare untyped
...
3. the single `(anyarray, anyarray)` overload must unify the two element types: a **non-array** operand
   (`5 @> ARRAY[1]`) or an **element-type mismatch** (`i32[] @> text[]`) matches no overload and is
...
- A **NULL array operand** (the whole value, e.g. `NULL::i32[]`) → **NULL** (the standard strict
...
1. the array operand must resolve to an **array** type `E[]`. A **non-array** right side (`1 = ANY(5)`)
   ...
   **bare untyped `NULL`** operand (`1 = ANY(NULL)`) leaves the array type undeterminable → **`42P18`**
   ...
   it, a documented degenerate divergence, out of corpus). A **typed** NULL array (`NULL::i32[]`)
...
`arg_families = ["any"]`, `result = "i32"`, `null = "none"`). A call takes one of two shapes:
...
- **VARIADIC-array form** `num_nulls(VARIADIC arr)` — `arr` must be an **array**; its **flattened
  ...
  the values to count. A **NULL whole-array** argument (`num_nulls(VARIADIC NULL::i32[])`) yields
...
4. **Spread form** (flag clear): the call must have **≥ `k`** arguments (`< k` → `42883`, so
   ...
   arrays included).
5. the **result** is the catalog `result` code (`i32`), independent of the arguments.
...
- **spread form** (`array_form` clear): count the arguments whose value is NULL (`num_nulls`) / non-NULL
  (`num_nonnulls`); return `i32`. Never NULL.
- **VARIADIC-array form** (`array_form` set): evaluate the single operand; if it is NULL return NULL;
  otherwise count its **flattened elements'** null-ness; return `i32`.
