...
- **File** — the open file kept for the handle's lifetime (today `open` reads all bytes then
  ...
  Rust safe `FileExt` positioned reads on Unix/Windows (with a serialized `File::seek+read`
  fallback where the standard library lacks one), Go `os.File.ReadAt`, TS Node `fs.readSync` at an offset; the
