...
> the relevant canonical document as it is made. **Do not delete this file until the maintainer
> explicitly requests deletion or every P0–P10 item is complete** and all remaining follow-ons,
> if any, have been folded back into `TODO.md`.
...
- [x] Partition the logical join tree into reorderable INNER/CROSS islands separated by outer-join,
  LATERAL, correlation, and other semantic barriers.
- [x] Specify and implement the bounded search algorithm chosen in P0 (for example, capped dynamic
  programming with a deterministic fallback).
- [x] Include physical access path and join algorithm in each partial-plan state rather than choosing
  them independently after join order.
- [x] Apply canonical state deduplication and tie-breaking independent of hashmap order.
- [x] Carry estimated rows/cost forward at each left-deep step.
- [x] Admit an INL edge only after its required sibling relation is present in the left prefix.
- [x] Preserve resolved expression slots through a physical relation permutation/mapping.
- [x] Re-run ORDER BY, scan-order, and LIMIT/top-N decisions on the final physical tree.
- [x] Add 3+-relation tests covering disconnected CROSS products, selective predicates, competing
  indexes, INL dependencies, hash choices, ties, barriers, NULLs, and empty relations.
- [x] Add a dedicated N-way NoREC/metamorphic relation comparing reordered and deliberately
  non-reorderable equivalent forms.
- [x] Add planning-limit boundary tests proving large FROM lists remain deterministically bounded.
- [x] Re-pin actual costs and benchmark representative 3-, 5-, and cap-boundary joins.
...
**Status:** implemented and verified on 2026-07-13. Maximal all-base INNER/CROSS islands jointly
select physical order, access paths, and join algorithms. Deterministic Pareto-frontier left-deep
DP covers through eight movable relations; larger islands use deterministic cheapest-next
construction. Outer/dependency inputs are hard fences, authored ON trees stay intact, and N-way
ordered LIMIT discounts only the final streamed join step. Shared plan/cost/cap/barrier coverage,
the `cost_plan_p8` NoREC relation, and 3-/5-/9-way benchmark lanes pin the slice.
...
### P9 human decision checkpoint

Approved by the maintainer on 2026-07-13:

- [x] **SQL surface:** one target per statement,
  `ANALYZE table [(column, ...)]`; ordinary qualified resolution admits main, temporary, and
  attached tables. Database-wide/multi-target ANALYZE and PG options are deferred.
- [x] **Authorization/transaction shape:** ANALYZE is a transactional write, is `25006` in a
  read-only transaction, and requires target-table `SELECT` plus `allow_ddl`; no new `MAINTAIN`
  privilege in this slice.
- [x] **Collection:** scan each requested column independently in deterministic storage-key order;
  collect exact NULL/width facts, a 30,000-row FNV-1a storage-key priority sample, and a bounded
  4,096-entry KMV NDV estimate. Planning remains I/O-free and collection uses no entropy seam.
- [x] **Target:** fixed statistics target 100, at most 100 MCVs and 101 histogram bounds; per-column
  target configuration is deferred.
- [x] **Types/width:** NULL facts for every type; NDV/MCV/histograms initially only for canonically
  key-encodable scalars and ranges, collation-aware for text. Persisted statistic values cap at
  128 encoded bytes; composite/array/json/jsonb distribution statistics are deferred.
- [x] **Staleness scaling:** retain statistics across row writes until explicit ANALYZE instead of
  discarding them. Frequency facts scale from `analyzed_rows` to current row count. NDV follows
  PostgreSQL's mixed rule: values above 10% of analyzed non-NULL rows scale proportionally; lower
  cardinalities stay fixed, and both forms clamp to the estimated current non-NULL population.
- [x] **Persistence:** format v29 kind-tagged catalog statistics records, fragmentable as bounded
  summary/MCV/histogram entries; no second root or new page type. Validate corruption and add
  byte-exact Ruby/Rust/Go/TypeScript fixtures.
- [x] **Estimator consumption:** statistics refine literal equality/IN/range/NULL predicates,
  generic parameter equality, equality joins, simple-column GROUP BY/DISTINCT, and variable-width
  hash work. Pattern/false-positive, multi-column-correlation, and MCV-aware join-skew models stay
  deferred.
- [x] **Resource accounting:** add a `statistics_value` runtime unit per canonical key byte inspected,
  beside ordinary scan/decompression charges; abort rolls back partial collection.
- [x] **Introspection:** add a summary-only `jed_statistics` catalog relation; detailed typed
  MCV/histogram arrays stay internal and plan effects remain visible through EXPLAIN.
- [x] **Automation:** explicit ANALYZE only; no background/automatic analyze or DML threshold.

- [x] Choose the SQL-reachable collection surface, likely an `ANALYZE` vertical slice, and define
  its PostgreSQL relationship/divergences.
- [x] Define per-column facts: NULL count/fraction, distinct count, histogram bounds, most-common
  values/frequencies, and any value-width facts needed by existing runtime cost units.
- [x] Define exact versus sampled collection. If sampled, specify a cross-core-identical sampling
  algorithm and seed; do not use host randomness or iteration order.
- [x] Scan rows in deterministic storage-key order and canonicalize values with shared type rules.
- [x] Define histogram bucket count, boundary selection, duplicate handling, NULL handling,
  collation behavior, and open/container-type eligibility.
- [x] Define transactional update, rollback, persistence, invalidation, and deterministic staleness
  policy. Never use wall-clock age.
- [x] Add statistics to the file format with byte-exact fixtures and corruption validation.
- [x] Extend the prepared-plan fingerprint with the new statistics facts/version.
- [x] Feed NDV/histograms into the existing selectivity rules without altering arithmetic or final
  tie-breaking.
- [x] Add corpus cases where distributions with equal row counts choose different plans.
- [x] Re-pin costs, extend NoREC, and benchmark skewed/uniform distributions.
...
**Status:** complete on 2026-07-13. `ANALYZE table [(columns)]` collects transactional v29 column
statistics in all three cores using exact NULL/width facts, a deterministic 30,000-row priority
sample, and bounded KMV NDV estimation. Successful row writes retain and mark facts stale;
frequencies scale to the current population and NDV uses the approved fixed/proportional split.
The estimator consumes the facts for the approved predicates, joins, simple grouping/distinct, and
variable-width hash work; `jed_statistics`, authorization, collation-skew handling, prepared-cache
invalidation, corruption checks, shared conformance, PostgreSQL-oracle coverage, website docs, and
the format fixture are included. Native uniform/skew lanes returned identical cross-core checksums;
the skewed post-ANALYZE query improved by approximately 11.2x in Go, 7.7x in Rust, and 5.5x in
TypeScript. `bundle exec rake ci` and the website production build pass.
...
- [ ] Delete this temporary checklist only after explicit maintainer approval or once P0–P10 are
  complete and no cross-session handoff state remains.

## Cross-session handoff

Update this short block at the end of each work session.

- **Current slice:** P9 deterministic NDV/histogram statistics is complete; P10 is next.
- **Last completed checkpoint:** SQL-reachable, transactional, persisted v29 column statistics are
  collected and consumed identically by Rust, Go, and TypeScript. Stale facts survive row writes and
  use the approved fixed/proportional NDV rule; prepared plans, collation skew, corruption handling,
  introspection, estimator choices, actual costs, NoREC, and benchmark lanes agree.
- **Branch / completion:** `path-b-p0-estimator-contract`; P9 is complete and its landing commit is
  the branch head.
- **Verification last run:** `bundle exec rake ci` passed on 2026-07-13, including 343/343 memory and
  319/319 applicable disk corpus files per core, 772 Rust and 701 TypeScript unit tests, Go unit
  packages, CLI/Ruby/migration suites, all spec/format/codegen/format/lint gates, and the 20-seed
  all-core NoREC sweep (2,280/2,280 generated-file checks). The PostgreSQL ANALYZE oracle check and
  website production build passed; native Rust/Go/TypeScript P9 benchmark lanes returned identical
  checksums.
- **Known blockers or open decisions:** no open P9 product decision. The aggregate
  ...
  toolchain target
- **Next action:** begin P10 final verification, documentation audit, and cleanup while preserving
  this checklist until its deletion condition is satisfied.
