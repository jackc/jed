...
//!   - threaded   (Rust's native mode): one OS thread per worker over a cloned `SharedDb` handle
...
use jed::{ReadHandle, SharedDb, Value, WriteHandle};
...
fn setup(db: &SharedDb, f: &StressFile) -> Result<(), String> {
    let mut wh = db.write();
    for s in &f.setup {
...
fn check_final(db: &SharedDb, f: &StressFile) -> Result<(String, bool), String> {
...
fn run_writer(db: &SharedDb, stmts: &[String], iterations: i64) -> Result<(), String> {
...
fn run_threaded(db: &SharedDb, f: &StressFile) -> Result<i64, String> {
...
fn run_sequential(db: &SharedDb, f: &StressFile) -> Result<i64, String> {
