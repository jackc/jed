...
- [ ] **True streaming result cursor** — make `Rows` a **pull source** so the non-blocking single-table pipeline yields row-at-a-time and blocking operators buffer-then-stream their output, instead of materializing the whole result before the caller sees a row (today `exec_select_plan` returns a full `Vec<Vec<Value>>`). PG/SQLite-faithful: a pull executor + **PG-faithful cursor snapshot pinning** (the cursor registers in the reader-liveness watermark, releases on drain/close/Drop). Cost is **byte-identical under full drain** (the harness drains), so no new corpus capability; the binding cross-core rule is the **mirrored streaming loop** (the `max_cost` abort point stays identical). Distinct from the operator-spill item above — that bounds blocking *input*; this bounds *output* and adds the pull cursor (the VDBE-forward prerequisite). → [streaming.md](spec/design/streaming.md) _(size: XL; deps: pager (done), spill (done), watermark (done); §9/§13)_
  - [ ] **S1 — the `Cursor` seam (no observable change):** `exec_select_plan` returns a `Cursor` (only a `Buffered` shape, wrapping today's materialized `Vec`); `Rows` delegates `next`/`column_names`/`cost`/`close`. Pure refactor — results, cost, goldens byte-unchanged.
  - [ ] **S2 — the pull B-tree scan cursor:** convert the scan from push (`scan_range(visit)`) to a pull cursor (frame stack over the persistent map) in Rust/Go, a generator in TS.
  - [ ] **S3 — stream the non-blocking pipeline + snapshot pinning:** wire scan-cursor → resolve → WHERE → project into a `Streaming` cursor (folding in the existing LIMIT/PK/index short-circuit paths); land the cursor snapshot pin + watermark registration + `close` + end-of-block close. The harness drains + surfaces mid-drain errors.
  - [ ] **S4 — lazy output from blocking operators:** generalize `SortedRows::next()` so `Buffered` runs the blocking part then yields its buffer lazily (sort/distinct/agg/window/join/set-op).
  - [ ] **S5 (deferred) — lazy small-inline-column decode:** decode only touched columns of inline values (PG `slot_getsomeattrs` / SQLite `OP_Column`), cost-neutral, behind the existing touched-set mask (streaming.md §8).
