...
- [x] **`date`** — calendar date (i32 days), strict ISO literals + BC + `±infinity`, date PK (type code 16); date arithmetic (`date ± int`, `date − date`, `date ± interval`). A strict island — no implicit compare to timestamp. → [date.md](spec/design/date.md)
  - [ ] _follow-on:_ clock-relative literals (`today`/`tomorrow`/`now`/`epoch`); remaining date functions (`make_date`, `date_part`, `current_date`). _(The runtime text→date cast has landed — STABLE, un-indexable `42P17`.)_ → [date.md §6](spec/design/date.md)
