...
- [ ] **Temporary tables** — `CREATE [TEMP|TEMPORARY] TABLE` (+ `DROP`): session-local relations that make **zero writes to the database file** (held outside the serialized `Snapshot`, no `format_version` bump), bounded by a deterministic storage budget to keep the untrusted-SQL guarantee (§13). Namespace precludes overlaps (`42P07`); new code `54P03 temp_storage_limit_exceeded`; `allow_ddl` splits into `allow_ddl` / `allow_temp_ddl`. **Landed:** session-local temp tables, CREATE/DROP INDEX on a temp table, serial/IDENTITY, composite-typed columns; and the **temp-blockstore slice** — session-local temp rides a per-domain in-RAM `MemoryBlockStore` + pinned pager (like an in-memory database) with **within-session free-list compaction** (`maybe_compact`, watermark-gated, so a never-reopened store no longer leaks a page per commit) and a **page-based** `54P03` budget (committed `page_count × page_size`) — all 3 cores, result/cost/byte-neutral. A database-wide `SHARED` temp kind (former slice 2, two-root commit, `allow_shared_temp_ddl`, `shared_temp_mem`) shipped briefly and was **removed** (Slice 0 of the attached-databases plan, 2026-07-03) in favor of a `Database`-scoped in-memory **attachment** ([attached-databases.md §6](spec/design/attached-databases.md)). **Open:** **slice 3 — spill-to-disk** (now a temp-`BlockStore` swap + bounded pool, the flip already put temp on the seam). → [temp-tables.md](spec/design/temp-tables.md) _(size: L; deps: session model (done), storage seam (done))_
...
- [x] **B+tree reshape — one packed representation, one storage path** ✅ COMPLETE (`format_version` 24) — the
  ...
  - [x] **B3 — `MemoryBlockStore` + pinned pool** ✅ (in-memory databases): an in-memory database
    ...
    temp still rides the `Decoded` writer-scratch arm B4 keeps anyway — but the `SHARED` surface is now
    **slated for removal** (attached-databases.md §6, Slice 0), so it is **not** getting its own
    `MemoryBlockStore` flip.
...
- [ ] **Attached (linked) databases** — ⏳ **design landed ([attached-databases.md](spec/design/attached-databases.md)); Slice 0 + Slice 1a built; Slice 1b landed in the Go core (Rust/TS porting).** A host attaches other jed databases (files on disk, or pure in-memory) to an open `Database` handle under a name; SQL then references their tables by a **database-qualified name** (`reports.sales`), joins across them included. The SQLite `ATTACH` capability (§9), reshaped to jed's seams. **Load-bearing decisions:** attaching is a **host-API act, never SQL** (`db.attach`/`detach`, like `open`/`load_unicode_data` — SQL stays pure, §13; SQL gains only qualified *names*); the qualifier is a **database**, not a schema (jed has no schemas); **no silent shadowing** (unqualified = implicit `main`+temp scope; explicit attachments reached by qualifier only — diverges from SQLite); the linkage is **never persisted** (files stay self-describing, §9/§13); and v1 allows **one durable writer per transaction** (multi-file atomic write deferred). **Reframes temp:** session-local temp becomes an implicit in-memory attachment, and the `SHARED` temp surface is **retired** (Slice 0). Slices: **0 — retire `SHARED` (subtractive) — LANDED**; **1** attached in-memory DBs + qualified names + reframe temp (no durability risk), building as **1a — `qualified_table` grammar + parser + `main`/`temp` resolution (LANDED, all 3 cores, `query.qualified_table`, no format bump)**, **1b — name→attachment registry + host-API in-memory `db.attach`/`detach` + the CREATE TABLE/INDEX qualifier (create-into-attachment) + N-root commit + cross-attachment joins + read-only mode (`25006`) + detach-in-use (`55006`); LANDED in the Go core (caps `attach.in_memory` + `harness.attach`, the `# attach:` corpus directive, no format bump — in-memory attachments are never serialized), Rust + TS porting**, **1c** reframe temp as an implicit attachment (deferred); **2** host-API file attach + cross-file read + single-database write; **3** (deferred) multi-file atomic write (2PC). → [attached-databases.md](spec/design/attached-databases.md) _(size: XL; deps: storage seam (done), session model (done), transactions/two-root commit (done); §9/§13)_
...
- [ ] **Unify the create constructor** — collapse the five overlapping DB constructors (in-memory `new`, in-memory-with-page-size, file `create`, file `open`, hidden open-with-options) to **two** — `create(opts)` (fresh, either backing; `opts.path` absent → in-memory) and `open(path, opts)` (existing file) — dropping the in-memory-specific constructors; each core's test suite keeps a private infallible `mem_db()`/`memDB()`/`memDb()` helper wrapping `create`. Designed in [api.md §2.1/§2.1.1](spec/design/api.md); host-API surface only, **byte-neutral, no format bump**. **Landed follow-ons:** (1) Go's `open` options are now exported — `OpenDatabaseWithOptions(path, OpenOptions)` + the `OpenOptions{ReadOnly, CacheBytes, WorkMem}` type (the mirror of Rust `open`/`open_with_options`), closing the last Go/Rust/TS open-surface divergence. (2) `DatabaseOptions`/`DefaultDatabaseOptions` unexported in all three cores (engine-internal only after `CreateOptions` replaced them; the OPFS host keeps its own still-consumed `opfs.ts` copy). **Deferred follow-ons:** add the anticipated create-time knobs (`memory_limit` first — the in-memory twin of `cache_bytes`; then a spill `temp_dir`, then a thread count) as new `CreateOptions` fields; optionally sweep the async OPFS/browser host (`createOpfs`/`OpfsDatabase.create`) into the unified `create(opts)` shape. → [api.md §2.1.1](spec/design/api.md) _(size: M; host-API only)_
