...
- **A deliberate scalar set**, starting small and growing on demand. Eventual intent:
  ...
  - **The type system is OPEN, not closed — composite (row) types have landed**
    ...
    slice** (no anonymous `record`), with composite-as-key deferred `0A000` (the interval-PK
    precedent — text/bytea/decimal keys have since landed, encoding.md §2.4/§2.5/§2.6) and no implicit
    per-table row types (a documented PG divergence).
