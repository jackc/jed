...
test("ON CONFLICT DO UPDATE assigning a PK column is 0A000", () => {
  ...
  assert.equal(
    errCode(() => execute(db, "INSERT INTO t VALUES (1, 5) ON CONFLICT (id) DO UPDATE SET id = excluded.id + 100")),
    "0A000",
  );
});
...
test("ON CONFLICT DO UPDATE SET col = DEFAULT is 42703", () => {
  const db = dbWith(["CREATE TABLE t (id i32 PRIMARY KEY, v i32 DEFAULT 7)", "INSERT INTO t VALUES (1, 10)"]);
  assert.equal(
    errCode(() => execute(db, "INSERT INTO t VALUES (1, 5) ON CONFLICT (id) DO UPDATE SET v = DEFAULT")),
    "42703",
  );
});
...
test("ON CONFLICT DO UPDATE assigning a GENERATED ALWAYS column is 428C9", () => {
  ...
  assert.equal(
    errCode(() => execute(db, "INSERT INTO t (k, v) VALUES (1, 5) ON CONFLICT (k) DO UPDATE SET id = 99")),
    "428C9",
  );
});
...
test("ON CONFLICT affected-row counts", () => {
  const db = dbWith(["CREATE TABLE t (id i32 PRIMARY KEY, v i32)", "INSERT INTO t VALUES (1, 10), (2, 20)"]);
  ...
  assert.equal(affected(db, "INSERT INTO t VALUES (2, 22), (4, 40) ON CONFLICT (id) DO UPDATE SET v = excluded.v"), 2);
  ...
  assert.equal(affected(db, "INSERT INTO t VALUES (1, 7) ON CONFLICT (id) DO UPDATE SET v = excluded.v WHERE false"), 0);
});
