...
function checkNames(db: Handle, table: string): string[] {
...
test("DDL errors match PostgreSQL", () => {
  const db = dbWith([]);
  assert.equal(
    errCode(() => db.execute("CREATE TABLE x (a int CHECK (a + 1))")),
    "42804",
  );
  ...
  for (const sql of [
    ...
    const e = errInfo(() => db.execute(sql));
    assert.equal(e.code, "0A000", sql);
    assert.equal(e.message, "cannot use subquery in check constraint");
  }
  let e = errInfo(() => db.execute("CREATE TABLE x (a int CHECK (sum(a) > 0))"));
  assert.equal(e.code, "42803");
  assert.equal(e.message, "aggregate functions are not allowed in check constraints");
  e = errInfo(() => db.execute("CREATE TABLE x (a int CHECK (a > $1))"));
  assert.equal(e.code, "42P02");
  assert.equal(e.message, "there is no parameter $1");
  assert.equal(
    errCode(() => db.execute("CREATE TABLE x (a int CHECK (nope > 0))")),
    "42703",
  );
  assert.equal(
    errCode(() => db.execute("CREATE TABLE x (a int CHECK (other.a > 0))")),
    "42P01",
  );
  // A forward reference is fine (checks resolve after all columns are known); so is a
  // reference qualified by this table's name.
  db.execute("CREATE TABLE fwd (CHECK (b > 0), b int)");
  db.execute("CREATE TABLE q (a int CHECK (q.a > 0))");
  // Duplicate explicit name.
  e = errInfo(() =>
    db.execute("CREATE TABLE x (a int CONSTRAINT cc CHECK (a > 0) CONSTRAINT cc CHECK (a < 5))"),
  );
  ...
  assert.equal(
    errCode(() =>
      db.execute("CREATE TABLE tb (a int CHECK (a > 0), CONSTRAINT tb_a_check CHECK (a < 5))"),
    ),
    "42710",
  );
  // PRIMARY KEY constraints resolve before any check expression (PG's order).
  e = errInfo(() => db.execute("CREATE TABLE tc (a int CHECK (nope > 0), PRIMARY KEY (alsonope))"));
  ...
  db.execute("CREATE TABLE t7 (a int DEFAULT -5 CHECK (a > 0))");
  // CHECK () is a syntax error.
  assert.equal(
    errCode(() => db.execute("CREATE TABLE x (a int, CHECK ())")),
    "42601",
  );
  // Columns may be NAMED check / constraint (the keywords stay non-reserved).
  db.execute("CREATE TABLE odd (check int, constraint i16)");
  db.execute("INSERT INTO odd VALUES (1, 2)");
});
...
test("violations match PostgreSQL order", () => {
  ...
  let e = errInfo(() => db.execute("INSERT INTO t VALUES (-1, 5)"));
  ...
  e = errInfo(() => db.execute("INSERT INTO t VALUES (-1, -5)"));
  assert.ok(e.message.endsWith("t_a_check"), e.message);
  e = errInfo(() => db.execute("INSERT INTO t VALUES (5, 1)"));
  assert.ok(e.message.endsWith("t_check"), e.message);
  e = errInfo(() => db.execute("INSERT INTO t5 VALUES (-1)"));
  ...
  assert.equal(
    errCode(() => db.execute("INSERT INTO tn VALUES (NULL)")),
    "23502",
  );
  // CHECK fires before the duplicate-key check.
  db.execute("INSERT INTO tu VALUES (1, 5)");
  assert.equal(
    errCode(() => db.execute("INSERT INTO tu VALUES (1, -1)")),
    "23514",
  );
  // A runtime error inside a check propagates as itself.
  db.execute("CREATE TABLE dz (a int CHECK (10 / a > 0))");
  assert.equal(
    errCode(() => db.execute("INSERT INTO dz VALUES (0)")),
    "22012",
  );
});
...
test("two-phase pass and defaults", () => {
  ...
  assert.equal(
    errCode(() => db.execute("INSERT INTO t SELECT v FROM src")),
    "23514",
  );
  // UPDATE: a later row violates → no row changes.
  db.execute("INSERT INTO t VALUES (1), (2)");
  assert.equal(
    errCode(() => db.execute("UPDATE t SET a = a - 1")),
    "23514",
  );
  ...
  assert.equal(
    errCode(() => db.execute("INSERT INTO t7 VALUES (DEFAULT, 1)")),
    "23514",
  );
  assert.equal(
    errCode(() => db.execute("INSERT INTO t7 (b) VALUES (1)")),
    "23514",
  );
  db.execute("INSERT INTO t7 VALUES (2, 1)");
});
...
test("the full expression surface works inside a check", () => {
  ...
  const e = errInfo(() => db.execute("INSERT INTO e VALUES (1, TRUE, 'a', 0.49)"));
...
test("check evaluation is metered", () => {
  ...
  db.setMaxCost(2n);
  assert.equal(
    errCode(() => db.execute("INSERT INTO c VALUES (4), (5), (6)")),
    "54P01",
  );
  db.setMaxCost(0n);
...
test("round-trips through the on-disk image", () => {
  ...
  const e = errInfo(() => loaded.execute("INSERT INTO t VALUES (4, -1, 1.00, 'ok')"));
  assert.equal(e.code, "23514");
  assert.equal(e.message, "new row for relation t violates check constraint t_b_check");
  assert.equal(
    errCode(() => loaded.execute("INSERT INTO t VALUES (4, 1, 0.10, 'ok')")),
    "23514",
  );
  assert.equal(
    errCode(() => loaded.execute("INSERT INTO t VALUES (4, 1, 1.00, 'nope')")),
    "23514",
  );
  loaded.execute("INSERT INTO t VALUES (4, 1, 1.00, 'a''b')");
  // A second generation (load → image → load) is byte-stable: the text is written back
  // verbatim.
  const reloaded = Database.fromImage(loaded.toImage(256, 1n));
  ...
  assert.equal(
    errCode(() => Database.fromImage(corrupt)),
    "XX001",
  );
});
