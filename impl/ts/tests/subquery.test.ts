...
import { dbWith, errCode, queryOutcome } from "./util.ts";
...
test("a subquery's cost is added once, the folded constant a leaf", () => {
  const db = ab();
  const base = queryOutcome(db, "SELECT id FROM a WHERE k = 999").cost;
  const withSub = queryOutcome(db, "SELECT id FROM a WHERE k = (SELECT max(k) FROM b)").cost;
...
test("a correlated mutation subquery's cost is per row, not folded once", () => {
  ...
  const corr = queryOutcome(
    ab(),
    "DELETE FROM a WHERE EXISTS (SELECT 1 FROM b WHERE b.k = a.k)",
  ).cost;
  const uncorr = queryOutcome(ab(), "DELETE FROM a WHERE k IN (SELECT k FROM b)").cost;
