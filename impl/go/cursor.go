...
//   - deferredCursor (executor.go) — a top-level set operation / pure-query WITH (streaming.md §7): it
//     defers the whole run to the first pull (over a frozen snapshot), then yields the result one row at
//     a time. The win is lazy-yield only (the work is all-at-once on the first pull, so an early exit
//     does not charge less); a mid-run abort surfaces during iteration, like the others.
