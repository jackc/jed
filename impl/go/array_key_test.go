...
func TestCompositeElementArrayKeysRejected(t *testing.T) {
	...
	if _, err := execute(db, "CREATE TABLE ok (id i32 PRIMARY KEY, k f32[] UNIQUE)"); err != nil {
		t.Fatalf("f32[] UNIQUE: %v", err)
	}
	if _, err := execute(db, "CREATE TABLE ok2 (id i32 PRIMARY KEY, k f64[])"); err != nil {
		t.Fatal(err)
	}
	if _, err := execute(db, "CREATE INDEX ix ON ok2 (k)"); err != nil {
