...
//!
//! jed types a bare integer literal / `ARRAY[…]` constructor as `int64`, so the tests pair bare
//! arrays with `int64[]` casts (matching element types); the element hint comes from the FIRST array
...
fn contains_basic() {
    ...
    assert_eq!(val(&mut db, "SELECT ARRAY[1,2,3] @> '{}'::int64[]"), "true");
    assert_eq!(val(&mut db, "SELECT '{}'::int64[] @> ARRAY[1]"), "false");
    assert_eq!(
        val(&mut db, "SELECT '{}'::int64[] @> '{}'::int64[]"),
        "true"
    );
...
fn contained_by_is_swapped_contains() {
    ...
    assert_eq!(val(&mut db, "SELECT '{}'::int64[] <@ ARRAY[1]"), "true");
}
...
fn overlaps_basic() {
    ...
    assert_eq!(val(&mut db, "SELECT ARRAY[1,2] && '{}'::int64[]"), "false");
}
...
fn strict_null_element_matching() {
    ...
    assert_eq!(
        val(&mut db, "SELECT ARRAY[1,2,NULL] @> '{NULL}'::int64[]"),
        "false"
    );
    assert_eq!(
        val(&mut db, "SELECT ARRAY[1,2,3] @> '{NULL}'::int64[]"),
        "false"
    );
    assert_eq!(
        val(
            &mut db,
            "SELECT '{NULL,NULL}'::int64[] @> '{NULL}'::int64[]"
        ),
        "false"
    );
    assert_eq!(
        val(&mut db, "SELECT ARRAY[1,NULL] && '{NULL}'::int64[]"),
        "false"
    );
...
fn null_whole_array_propagates() {
    ...
    assert_eq!(val(&mut db, "SELECT NULL::int64[] @> ARRAY[1]"), "NULL");
    assert_eq!(val(&mut db, "SELECT ARRAY[1] @> NULL::int64[]"), "NULL");
    assert_eq!(val(&mut db, "SELECT NULL::int64[] && ARRAY[1]"), "NULL");
    assert_eq!(val(&mut db, "SELECT ARRAY[1] <@ NULL::int64[]"), "NULL");
}
...
fn literal_adaptation_to_element_type() {
    ...
    assert_eq!(
        val(&mut db, "SELECT '{1,2,3}'::int32[] @> ARRAY[2]"),
        "true"
    );
    assert_eq!(
        val(&mut db, "SELECT '{2}'::int32[] <@ ARRAY[1,2,3]"),
        "true"
    );
}
