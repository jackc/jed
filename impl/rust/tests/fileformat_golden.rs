...
fn pk_table_db() -> Session {
    let mut db = Database::new_in_memory_with_page_size(GOLDEN_PAGE_SIZE).session(SessionOptions::default());
...
fn write_matches_goldens() {
    ...
    let cases: &[(&str, Builder)] = &[
        (
            "empty_db.jed",
            (|| Database::new_in_memory().session(SessionOptions::default())) as Builder,
        ),
...
fn read_goldens_reproduces_rows() {
    ...
    let empty = Database::from_image(&fixture("empty_db.jed")).unwrap().session(SessionOptions::default());
...
fn read_golden_reconstructs_catalog() {
    let loaded = Database::from_image(&fixture("pk_table.jed")).unwrap().session(SessionOptions::default());
...
fn default_survives_load() {
    let mut loaded = Database::from_image(&fixture("default_table.jed")).unwrap().session(SessionOptions::default());
...
fn rowid_counter_survives_serialize_and_load() {
    ...
    loaded.execute("INSERT INTO r VALUES (10, 100)", &[]).expect("insert after load");
...
fn round_trip_at_default_page_size() {
    let mut db = Database::new_in_memory_with_page_size(8192).session(SessionOptions::default());
    run(&mut db, "CREATE TABLE t (id i32 PRIMARY KEY, v i16)");
    for i in 1..=20i64 {
        let v = if i == 3 {
            "NULL".to_string()
    ...
    let loaded = Database::from_image(&image).unwrap().session(SessionOptions::default());
    assert_eq!(
