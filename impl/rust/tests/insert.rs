...
fn boundary_values_round_trip() {
    ...
    db.query_outcome("INSERT INTO t VALUES (1, 32767, 9223372036854775807)", &[])
        .unwrap();
    db.query_outcome(
...
fn int32_and_int64_overflow_boundaries() {
    ...
    assert_eq!(
        db.query_outcome("INSERT INTO t VALUES (1, 2147483648)", &[])
            .unwrap_err()
            .code(),
        "22003"
    );
    // i32 max fits.
    assert_eq!(
        db.query_outcome("INSERT INTO t VALUES (2, 2147483647)", &[])
            .unwrap(),
        Outcome::Statement {
            cost: 0,
...
fn insert_into_missing_table_traps() {
    let mut db = Database::create(CreateOptions::default())
        ...
        .session(SessionOptions::default());
    let err = db
        .query_outcome("INSERT INTO nope VALUES (1)", &[])
        .unwrap_err();
...
fn no_pk_multi_row_insert_keeps_insertion_order() {
    ...
    db.query_outcome("INSERT INTO log VALUES (30), (10), (20)", &[])
        .unwrap();
...
fn no_pk_multi_row_insert_is_all_or_nothing() {
    ...
    db.query_outcome("INSERT INTO log VALUES (1)", &[]).unwrap();
    ...
    let err = db
        .query_outcome("INSERT INTO log VALUES (2), (99999)", &[])
        .unwrap_err();
    assert_eq!(err.code(), "22003");
    db.query_outcome("INSERT INTO log VALUES (3), (4)", &[])
        .unwrap();
    let vals: Vec<Value> = db
        .rows_in_key_order("log")
        .unwrap()
...
fn insert_select_cost_is_the_embedded_select_cost() {
    ...
    assert_eq!(
        db.query_outcome("INSERT INTO dst SELECT id, a, b FROM src", &[])
            .unwrap(),
        Outcome::Statement {
            cost: 7,
