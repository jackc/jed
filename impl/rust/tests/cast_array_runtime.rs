...
fn scalar(db: &mut Session, expr: &str) -> String {
    match db.query_outcome(&format!("SELECT {expr}"), &[]).unwrap() {
...
fn array_to_text_is_explicit_only() {
    ...
    db.query_outcome("CREATE TABLE t (id i32 PRIMARY KEY, label text)", &[])
        .unwrap();
    ...
    db.query_outcome("INSERT INTO t VALUES (1, '{1,2,3}')", &[])
        .unwrap();
    assert_eq!(
...
fn runtime_text_to_float_arrays() {
    ...
    db.query_outcome("CREATE TABLE t (id i32 PRIMARY KEY, s text)", &[])
        .unwrap();
    db.query_outcome("INSERT INTO t VALUES (1, '{0.5,0.25,-1.5}')", &[])
        .unwrap();
    // text → f64[] (binary64-exact values render exactly).
    let got = match db
        .query_outcome("SELECT (s::float8[])::text FROM t WHERE id = 1", &[])
        .unwrap()
    {
        Outcome::Query { rows, .. } => rows[0][0].render(),
        other => panic!("{other:?}"),
