...
fn identity_desugars_to_owned_sequences() {
    ...
    db.execute("CREATE TABLE t (id int GENERATED ALWAYS AS IDENTITY PRIMARY KEY, \
         n int GENERATED BY DEFAULT AS IDENTITY, v text)", &[])
    .unwrap();
    match db.execute("INSERT INTO t (v) VALUES ('a'), ('b') RETURNING id, n", &[])
    .unwrap()
    {
        Outcome::Query { rows, .. } => {
...
fn identity_kind_survives_reopen() {
    ...
    let _ = std::fs::remove_file(&path);
    {
        ...
        db.execute("CREATE TABLE t (id int GENERATED ALWAYS AS IDENTITY, \
             n int GENERATED BY DEFAULT AS IDENTITY, v text)", &[])
        .unwrap();
    ...
    }
    {
        ...
        db.execute("INSERT INTO t (id, v) OVERRIDING SYSTEM VALUE VALUES (5, 'x')", &[])
        .unwrap();
        // BY DEFAULT still accepts an explicit value with no override.
        db.execute("INSERT INTO t (n, v) VALUES (9, 'y')", &[]).unwrap();
...
fn identity_owned_sequences_auto_drop() {
    let mut db = Database::new_in_memory().session(SessionOptions::default());
    db.execute("CREATE TABLE t (id int GENERATED ALWAYS AS IDENTITY, n bigint GENERATED BY DEFAULT AS IDENTITY)", &[])
    .unwrap();
    assert_eq!(err_code(&mut db, "DROP SEQUENCE t_id_seq"), "2BP01");
...
fn identity_owned_sequence_follows_column_type() {
    ...
    db.execute("SELECT setval('t_id_seq', 32767)", &[]).unwrap();
...
fn identity_with_sequence_options() {
    let mut db = Database::new_in_memory().session(SessionOptions::default());
    db.execute("CREATE TABLE t (id int GENERATED ALWAYS AS IDENTITY (START WITH 100 INCREMENT BY 5), v text)", &[])
    .unwrap();
    match db.execute("INSERT INTO t (v) VALUES ('a'), ('b') RETURNING id", &[])
    .unwrap()
    {
        Outcome::Query { rows, .. } => {
