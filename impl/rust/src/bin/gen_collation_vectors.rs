...
fn main() {
    ...
    {
        let root = compile(&["collation/fixtures/dev-root.allkeys"], "dev-root");
        ...
        let mut out = String::new();
        out.push_str("# Collation JUCD bundle vectors (spec/collation/README.md §5). GENERATED by\n");
        out.push_str(
            "# impl/rust/src/bin/gen_collation_vectors.rs; cross-confirmed byte-for-byte by all\n",
        );
        out.push_str(
            "# three cores (CLAUDE.md §8). Do not hand-edit. The harness rebuilds the bundle from\n",
        );
        out.push_str(
            "# def_files (build_bundle: shared root + per-locale deltas), asserts save_bundle ==\n",
        );
        out.push_str(
            "# bundle_hex, round-trips open_bundle, and checks merge(root, delta) reproduces the\n",
        );
        out.push_str(
            "# full tailoring table (§5.1). DEV fixtures only — small enough to pin as full hex.\n\n",
        );
        out.push_str("schema_version = 1\n\n");
        out.push_str("[[bundle]]\n");
        ...
        out.push_str(&format!(
            "root_def_files = [{}]\n",
            files_toml(&["collation/fixtures/dev-root.allkeys"])
        ));
        out.push_str(&format!("bundle_hex = \"{}\"\n", hex(&bytes)));
        out.push_str("\n[[bundle.tailoring]]\n");
        out.push_str("name = \"dev-nordic\"\n");
        out.push_str(&format!("def_files = [{}]\n", files_toml(nordic_files)));
