...
pub const SUPPORTED_CAPABILITIES: &[&str] = &[
    ...
    // lives in the temp snapshot — zero file writes — and is auto-dropped with the table), and
    ...
    // (spec/design/temp-tables.md §8). The capability gate is session.allow_temp_ddl; the storage budget
    ...
    "query.index_partial",
    // EXPLAIN — render the selected plan as deterministic
    // depth/node/detail/est_rows/est_cost rows without executing it (spec/design/explain.md).
    "query.explain",
    ...
    // Set-returning functions in FROM: generate_series, a computed row source, including
    // fixed-output column aliases `AS g(c1, …)` (functions.md §10, grammar.md §35).
    "query.set_returning",
